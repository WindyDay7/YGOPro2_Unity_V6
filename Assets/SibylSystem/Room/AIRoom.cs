...
public class AIRoom : WindowServantSP
{
    ...
    private static string GetAiServerExecutable()
    {
#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN
        return "AI.Server.exe";
    ...
    private static string GetWindBotExecutable()
    {
#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN
        return Path.Combine("WindBot", "WindBot.exe");
#elif UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX || UNITY_STANDALONE_LINUX
        return Path.Combine("WindBot", "WindBot");
#else
        return null;
#endif
    }

    private static bool TryGetAiExecutables(out string serverExecutable, out string botExecutable)
    {
        serverExecutable = GetAiServerExecutable();
        botExecutable = GetWindBotExecutable();
        if (string.IsNullOrEmpty(serverExecutable) || string.IsNullOrEmpty(botExecutable))
        {
            return false;
        }
        return File.Exists(serverExecutable) && File.Exists(botExecutable);
    }

    private static int GetAiServerPortBase()
    {
        int parsed;
        if (!int.TryParse(Config.Get("aiServerPortBase", "17911"), out parsed))
        {
            parsed = 17911;
        }
        return parsed;
    ...
    public void launch(string command, bool lockhand, bool nocheck, bool noshuffle)
    {
        ...
        if (!TryGetAiExecutables(out serverExecutable, out botExecutable))
        {
            RMSshow_none(InterString.Get("当前平台未找到可用的人机二进制。@nWindows 需要 AI.Server.exe 与 WindBot.exe；macOS 需要可执行的 AI.Server 与 WindBot。"));
            return;
        }
        command = command.Replace("'", "\"");
        if (lockhand) command += " Hand=1";
        int aiServerPortBase = GetAiServerPortBase();

        serverProcess = new System.Diagnostics.Process();
        serverProcess.StartInfo.UseShellExecute = false;
        serverProcess.StartInfo.FileName = serverExecutable;
        serverProcess.StartInfo.Arguments = aiServerPortBase.ToString() + " -1 5 0 F " + (nocheck ? "T" : "F") + " " + (noshuffle ? "T" : "F") + " 8000 5 1 0 0";
    ...
    botProcess.StartInfo.WorkingDirectory = Path.GetDirectoryName(botExecutable);
